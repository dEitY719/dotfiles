...
_generate_pg_service_conf() {
    local now
    now=$(date)
    printf '# Autogenerated by postgresql.sh — %s\n# Do not edit manually. Edit %s instead.\n\n' \
        "$now" "$PG_SERVICES_FILE" >"$PGSERVICE_CONF"
    ...
    for entry in "${services[@]}"; do
        ...
        if [[ "$svc" =~ ^[a-zA-Z0-9_-]+$ ]]; then
            printf '[%s]\nhost=%s\nport=%s\ndbname=%s\nuser=%s\npassword=%s\n\n' \
                "$svc" "$DEFAULT_HOST" "$DEFAULT_PORT" "$db" "$user" "$pass" >>"$PGSERVICE_CONF"
        fi
    done
}
