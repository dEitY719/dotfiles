...
gc_help() {
    ...
    echo ""
    echo "  ${bold}상황${reset}: 다른 팀원이 만든 프로젝트에 처음 join"
    echo ""
    echo "  ${bold}Step 1️⃣  (프로젝트 소유자가 실행 - 1회만)${reset}"
    echo "  ─────────────────────────────────"
    echo "    1. 당신의 GPG 공개키 받기:"
    echo "       ${bold}당신: gpg --export YOUR_EMAIL | 프로젝트 소유자에게 전달${reset}"
    echo ""
    ...
    echo "  ──────────────────────────────────"
    echo "    ${bold}git clone <repo> && cd <repo>${reset}"
    echo "    ${bold}git pull${reset}"
    echo "    ${bold}git-crypt unlock${reset}"
    echo "    ${bold}cat .env  # 성공 확인${reset}"
    echo ""

    ux_section "Symmetric Key로 Join (소유자가 key 제공)"
    echo ""
    echo "  ${bold}Step 1️⃣  (프로젝트 소유자가 1회만)${reset}"
    echo "  ─────────────────────────────────"
    echo "    ${bold}git-crypt export-key ~/repo-key.txt${reset}"
    echo "    👉 이 파일을 안전하게 당신에게 전달"
    echo ""
    echo "  ${bold}Step 2️⃣  (당신이 받은 후)${reset}"
    echo "  ──────────────────────────────"
    echo "    ${bold}git clone <repo> && cd <repo>${reset}"
    echo "    ${bold}git-crypt unlock ~/repo-key.txt${reset}"
    echo ""
