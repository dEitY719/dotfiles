...
claude_help() {
    ...
    ux_section "Organization Usage & Cost Reports (Admin API)"
    ux_info "📊 Usage Report: ${UX_SUCCESS}usage_report${UX_RESET}"
    echo "  Shows token usage data for entire organization"
    echo "  Usage: ${UX_SUCCESS}usage_report [START_DATE] [END_DATE] [GROUP_BY]${UX_RESET}"
    echo "  Default: today's data, grouped by model"
    echo "  Example: ${UX_SUCCESS}usage_report 2025-01-10 2025-01-15 model${UX_RESET}"
    echo ""
    ux_info "💰 Cost Report: ${UX_SUCCESS}cost_report${UX_RESET}"
    echo "  Shows USD cost breakdown for entire organization"
    echo "  Usage: ${UX_SUCCESS}cost_report [START_DATE] [END_DATE]${UX_RESET}"
    echo "  Default: yesterday and today"
    echo "  Example: ${UX_SUCCESS}cost_report 2025-01-10 2025-01-15${UX_RESET}"
    echo ""
    ux_info "⚙️ Setup Required:"
    echo "  export ANTHROPIC_ADMIN_API_KEY=\"sk-ant-admin-...\""
    echo ""
    ux_info "💡 Shorthand Functions:"
    echo "  ${UX_SUCCESS}usage${UX_RESET}     # Shorthand for usage_report"
    echo "  ${UX_SUCCESS}cost${UX_RESET}      # Shorthand for cost_report"
    echo ""
    ux_info "📋 Help Commands:"
    echo "  ${UX_SUCCESS}usage_report --help${UX_RESET}  # Show usage report help"
    echo "  ${UX_SUCCESS}cost_report --help${UX_RESET}   # Show cost report help"
    echo ""
