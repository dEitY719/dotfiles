...
# ~/dotfiles/zsh/ux_lib/ux_lib.zsh
# Central UX library for consistent styling across all dotfiles scripts
# Zsh-compatible version ported from bash/ux_lib/ux_lib.bash
...
# In zsh, we use ${(%):-%N} to get the sourced script name
UX_LIB_DIR="${0:h}"
...
ux_spinner() {
    ...
    while kill -0 "$pid" 2>/dev/null; do
        printf "\r%s%s%s %s..." "${UX_INFO}" "${frames[$((i+1))]}" "${UX_RESET}" "$message"
        i=$(( (i + 1) % ${#frames[@]} ))
        sleep "$delay"
    done
...
# Display an interactive menu using a Python script or basic zsh fallback
