...
├── keybindings.json   # VS Code 키바인딩 설정 (Git에서 추적)
├── sync-push.sh       # Push: base.json & keybindings.json → VS Code 설정
├── sync-pull.sh       # Pull: VS Code 설정 → base.json & keybindings.json
└── README.md          # 이 파일
```

## 🔄 동기화 방법

### 1️⃣ PUSH: base.json & keybindings.json을 VS Code에 복사
...
**수행 작업:**
- ✅ `base.json` → VS Code `settings.json`에 복사
- ✅ `keybindings.json` → VS Code `keybindings.json`에 복사
- ✅ 기존 설정 자동 백업 생성
- ✅ VS Code 재시작 필요

---

### 2️⃣ PULL: VS Code 설정을 base.json & keybindings.json에 복사
...
**수행 작업:**
- ✅ VS Code `settings.json` → `base.json` 복사
- ✅ VS Code `keybindings.json` → `keybindings.json` 복사 (존재하는 경우)
- ✅ 기존 파일들 자동 백업 생성
...
#### 외부 PC에서:
```bash
# 1. VS Code 설정을 .vscode/로 저장
./.vscode/sync-pull.sh

# 2. 확인 후 Git에 커밋
git add .vscode/base.json .vscode/keybindings.json
...
.vscode/keybindings.json.backup.20250120_143022

# PUSH 실행 시
$APPDATA\Code\User\settings.json.backup.20250120_143022
$APPDATA\Code\User\keybindings.json.backup.20250120_143022
```
...
### 1. 민감한 정보는 저장하지 마세요
다음 항목은 `base.json`이나 `keybindings.json`에 포함하면 안 됩니다:
- 개인 토큰, API 키
- 절대 경로
- 로컬 환경 경로
- 회사 특화 설정 (회사 환경 설정은 동기화하지 않음)
...
# 현재 keybindings.json 내용 확인
cat .vscode/keybindings.json | python3 -m json.tool
...
# 백업 파일 목록 확인
ls -la .vscode/*.backup.*
ls -la "$APPDATA/Code/User/"*.backup.*  # Windows
```
...
```bash
# 1. 현재 VS Code 설정을 .vscode/로 저장 (처음 한 번)
./.vscode/sync-pull.sh
