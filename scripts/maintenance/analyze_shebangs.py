    ...
    for pattern in ['**/*.sh', '**/*.bash']:
        for filepath in Path(root_dir).glob(pattern):
            ...
            entry = {
                ...
                'features': features,
                'is_exec': is_exec,
                'priority': str(rel_path).startswith('shell-common/env/') or
                           str(rel_path).startswith('shell-common/functions/')
            }
...
def main():
    ...
    if not results['wrong_shebang'] and not results['missing_shebang']:
        ...
        if results['missing_shebang']:
            print(f"{Colors.BOLD}{Colors.YELLOW}Missing Shebang ({len(results['missing_shebang'])} files):{Colors.RESET}")
            print()
