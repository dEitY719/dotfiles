...
이 스크립트는 npm을 이용하여 Codex CLI를 제거합니다.
제거 과정:
  1. npm 설치 확인
  2. Codex CLI 제거
  3. 제거 확인
...
EOF
    ...
    # ========================================
    info "Step 2/3: Codex CLI 제거 중..."

    # OpenAI Codex npm 패키지
    local codex_package="@openai/codex"

    if confirm "Codex CLI (${codex_package})를 제거하시겠습니까?"; then
        npm uninstall -g "$codex_package" || {
            error "Codex CLI 제거 실패"
            return 1
        }
        success "Codex CLI 제거 완료"
    else
        warning "Step 2 스킵됨"
    fi
    ...
    if command -v codex &> /dev/null; then
        warning "codex 명령어가 여전히 존재합니다."
        warning "PATH 설정을 확인하고 터미널을 다시 시작하세요."
    else
        success "Codex CLI가 성공적으로 제거되었습니다."
    fi
