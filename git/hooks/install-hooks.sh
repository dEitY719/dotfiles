...
echo ""
echo "─────────────────────────────────────────────────────────────"
echo "Pre-push policy (this repo only, since issue #754)"
echo "─────────────────────────────────────────────────────────────"
echo "  The dotfiles repo itself runs the full pytest suite via the"
echo "  pre-push hook (\"mise run test\") instead of GitHub Actions."
echo "  This install script only wires post-commit for downstream"
echo "  projects — the pre-push hook here is managed via"
echo "  core.hooksPath set by ./setup.sh."
echo ""
echo "  Opt-out for the local pytest layer:"
echo "    SKIP_LOCAL_PYTEST=1 git push   # WIP push, intentional fail"
echo ""
echo "  WARNING:"
echo "    --no-verify bypasses ALL hook layers (protected-branch,"
echo "    leak guard, AND mise run test). Do NOT use --no-verify as"
echo "    a habit; prefer SKIP_LOCAL_PYTEST=1 (narrow opt-out) and"
echo "    fix the underlying test failure. SSOT:"
echo "    docs/.ssot/local-test-policy.md"
