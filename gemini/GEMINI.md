...
- Do not modify `[tool.black]` or `[tool.isort]` sections in `pyproject.toml` or `mise.toml` as they are working correctly.
...
- For code quality checks, use `mise run lint` (or `devx lint`); do not invoke ruff / mypy individually.
...
1. Linux(Ubuntu)
2. vscode IDE
3. mise.toml (ruff, mypy, shellcheck, shfmt)
4. pyproject.toml
