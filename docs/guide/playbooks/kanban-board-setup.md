...
실행 가이드다. 기본 경로는 `/gh-kanban-bootstrap` skill 1회 실행 (내부적으로
`claude/skills/gh-kanban-bootstrap/lib/setup.sh` 호출) 과 GitHub UI 체크
몇 단계이며, 수동 절차는 스크립트 미사용 시나 디버깅용 fallback 으로 남긴다.

본 문서는 dotfiles repo의 SSOT
(`docs/.ssot/github-project-board.md`)에서 **재사용 가능한 실행
...
- Issue 3단계: `Backlog → In progress → Done` (Issue 는 `In review` 에
  도달하지 않음 — #289).
...
```bash
# 경로 A (권장 — Claude Code/Codex/Gemini): skill 호출
/gh-kanban-bootstrap

# 경로 B (직접 호출 — 셸 어디서나): script 단일 SSOT
bash claude/skills/gh-kanban-bootstrap/lib/setup.sh

# 경로 C: 명시적 owner/repo
bash claude/skills/gh-kanban-bootstrap/lib/setup.sh --owner <owner> --repo <repo>
```

옵션 (script 기준 — skill 도 동일하게 패스스루):
...
> 이전 경로 (`scripts/` 하위 setup 스크립트) 는 issue #699 에서 삭제되고
> skill 내부 `lib/setup.sh` 로 이동했다. 호환성 stub 은 두지 않으므로 새
> 경로만 사용한다.
...
- `Pull request linked to issue`:
  **비활성화 (DISABLE)** — #289 결정. Issue 라이프사이클은 3단계
  (`Backlog → In progress → Done`) 이며 Issue 는 `In review` 에
  도달하지 않는다. 이 워크플로를 켜 두면 PR 이 연결되는 순간 Issue 카드가
  잘못 `In review` 로 이동하므로 disable 한다.
...
| 3  | `Pull request linked to issue`  | **disable (#289)** — Issue 는 `In review` 미방문 |
...
Issue 는 `In review` 에 도달하지 않는다 (#289) — 그 컬럼은 PR 전용이다.
...
- 본 playbook 출처 SSOT:
  [docs/.ssot/github-project-board.md](../.ssot/github-project-board.md)
...
- 자동화 진입점:
  - skill: `claude/skills/gh-kanban-bootstrap/SKILL.md` (`/gh-kanban-bootstrap`)
  - script (단일 SSOT): `claude/skills/gh-kanban-bootstrap/lib/setup.sh`
    (이전 위치는 issue #699 에서 이동 — 자세한 migration 노트는 SKILL.md 참고)
