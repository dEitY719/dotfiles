...
### 6. p10k frozen prompt — 1줄 진단과 캐시 cleanup 계약

**파일**: [`p10k-frozen-prompt-diagnosis.md`](./p10k-frozen-prompt-diagnosis.md)

`gwt spawn --launch` 직후 prompt 가 이전 디렉터리/브랜치에 frozen 될 때
`print -lr -- $precmd_functions` 한 줄로 `_p9k_precmd` 등록 여부를 진단.
복구는 `zsh-clear-p10k-caches` (instant-prompt + dump + .zwc + per-user
