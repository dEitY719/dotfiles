...
기본 동작은 위처럼 **add-only** (하위호환). `--sync` 를 주면 add/install 패스
이후 **prune 패스**가 추가로 돌아, SSOT 에 없는 잉여 로컬 항목을
`claude plugin uninstall` / `claude plugin marketplace remove` 로 제거해
로컬 상태를 manifest 와 정확히 일치시킨다. `--sync --dry-run` 은 prune 계획까지
미리보기한다.

- **로컬 ground truth**: hook 과 동일하게
  `~/.claude-shared/plugins/{known_marketplaces,installed_plugins}.json` 를 읽는다
  (테스트/오버라이드용 `CLAUDE_SHARED_PLUGINS_DIR`).
- **keep-set = 모든 in-scope SSOT 의 합집합(union)** — 공용 SSOT ∪ (internal +
  `company/.git` 일 때) company SSOT ∪ 화이트리스트. per-manifest 관할이 아니라
  union 으로 두는 것이 "공용 prune 패스가 company 전용 항목을 잉여로 오판" 하는
  문제(설계 논의 포인트)의 해결안이다.
- **prune 대상에서 자동 제외**:
  - `source:directory` 마켓플레이스/플러그인 — 재현 불가한 머신 로컬 (Non-Goal).
  - `scope:user` 가 아닌 플러그인 — hook 이 애초에 기록하지 않는 대상.
- **화이트리스트** `claude/plugin/.local-marketplaces.json` (gitignored,
  `{"marketplaces":[...],"plugins":[...]}`) — PC별 로컬 수동 추가 항목 보호.
  external/public PC 에서 로컬에 사내 항목(`aws-login` 등)이 남아있는 엣지
  케이스도 이 화이트리스트로 보호한다 (company SSOT 접근 불가라 union 에 못 넣음).
- 제거 순서: 플러그인을 먼저 uninstall 한 뒤 마켓플레이스를 remove — 마켓플레이스가
  먼저 사라져 플러그인이 orphan 되는 것을 방지.
