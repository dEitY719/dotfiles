...
Claude Code 세션 (Bash 도구로 claude plugin ... 실행, 어느 프로젝트에서든 — hook은 전역)
  ...
  PostToolUse hook: claude/hooks/plugin-sync.sh
        ...
        │     사내 → $COMPANY_SKILLS_HOME/claude/plugin/*.json (private repo, 즉시 로컬 커밋)
        ▼
  git history (public dotfiles + private company-skills, 각각 별도 커밋)

신규 PC:
  git clone dotfiles && cd dotfiles
  ./claude/plugin/restore.sh   ← 수동 1회 실행
        │  공용 manifest는 항상 복원
        │  ~/.dotfiles-setup-mode == internal 이면
        │    $COMPANY_SKILLS_HOME/claude/plugin/ 도 추가로 복원
        ▼
...
**사내 전용 (private, `$COMPANY_SKILLS_HOME/claude/plugin/`, dotfiles 트리 밖 별도 git 레포):**

```
$COMPANY_SKILLS_HOME/claude/plugin/
├── marketplaces.json   # { "<marketplace-name>": "<repo-or-url>", ... }  (source:github가 아닌 것)
└── plugins.json         # { "plugins": [...] }
```

`$COMPANY_SKILLS_HOME` (기본값 `~/para/project/company-skills`)는 이미
`scripts/setup-company-skills.sh`가 사내 전용 스킬 오버레이에 쓰는 것과 **동일한
프라이빗 레포**다. 새 디렉토리를 만들지 않고 그 레포의 `claude/plugin/` 하위에
얹는다 — dotfiles(퍼블릭)에는 사내 마켓플레이스 이름·저장소 경로·URL이 한 글자도
들어가지 않는다.
...
4. 결과를 공용/사내 경로에 각각 쓰고, 각 레포(`$HOME/dotfiles`,
   `$COMPANY_SKILLS_HOME`)에서 diff가 있을 때만 `git add` + `git commit`
   (메시지: `chore(claude-plugin): sync manifest`).
...
if [ "$MODE" = "internal" ]; then
    PRIV="${COMPANY_SKILLS_HOME:-$HOME/para/project/company-skills}/claude/plugin"
...
| `$COMPANY_SKILLS_HOME` 없음 (external/public PC) | 사내 분류 결과가 있어도 조용히 skip (애초에 네트워크상 사내 마켓플레이스를 설치할 수 없어 발생 안 함) |
