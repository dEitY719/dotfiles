...
```
PR  #42 (OPEN, review: APPROVED, updated 2026-04-29)
```

`gh pr view <N> --json state,reviewDecision,mergedAt,closedAt,updatedAt` 한 번 호출.
표시 날짜는 `mergedAt // closedAt // updatedAt` 우선순위로 선택 (OPEN PR은 `mergedAt`/`closedAt`이 null이므로 `updatedAt` fallback).
실패 시 `(unreachable — gh CLI failed)`로 표기, **verdict 로직엔 영향 없음**.
구현 시 `jq` 필터는 저장소 규칙에 따라 null-safe 패턴(`.field? // empty`)을 사용한다.
...
gh-flow와 동일 레이아웃에 `PR` (informational), `Flags` (신규), `AI` (신규) 행 추가:
...
`flags` 파일이 없으면 `Flags  (none)`. `ai` 파일이 없으면 `AI  (unknown)`.
