...
- [ ] **P0-1**: 커밋/브랜치에 티켓 키 포함 규칙 고정
  ```bash
  # 브랜치: proj-245-short-desc
  # 커밋: [PROJ-245] type: description
  ```
- [ ] **P0-2**: .gitmessage 템플릿 작성 & 적용
  ```bash
  git config commit.template .gitmessage
  ```
- [ ] **P0-3**: Post-commit hook 작성 & 테스트
  ```bash
  chmod +x .git/hooks/post-commit
  ```
- [ ] **P0-4**: 이번 작업에 적용 & 검증
...
**완료 기준**: work_log.txt에 자동 기록 확인

---

### P1 (중요, 1주 내): SSOT 저장소 설정

**목표**: rca-knowledge를 SSOT로 설정
**소요**: 1-2h
**효과**: 여러 프로젝트 통합 관리

- [ ] **P1-1**: rca-knowledge 디렉토리 구조 생성
  ```bash
  mkdir -p /home/bwyoon/para/archive/rca-knowledge/docs/analysis/{testing,infrastructure,documentation}
  ```
- [ ] **P1-2**: _index.json 초기화
  ```json
  {"version": "1.0", "documents": []}
  ```
- [ ] **P1-3**: 복붙용 템플릿 파일 생성 (부록B 기반)
  - docs/worklog/template.md
  - docs/jira/template.md
  - docs/confluence/template.md

**완료 기준**: 첫 문서가 rca-knowledge에 저장됨

---

### P2 (권장, 2-3주): 자동화 스킬 개발

**목표**: make-jira/make-confluence 반자동화
**소요**: 10h
**효과**: 자동화 90%

- [ ] **P2-1**: make-jira 스킬 설계
  - 입력: git log, commit message
  - 출력: Jira 주간보고 템플릿 (Markdown)
  - 저장: rca-knowledge/docs/analysis/jira-records/

- [ ] **P2-2**: make-confluence 스킬 설계
  - 입력: docs/technic/*.md
  - 출력: Confluence Tip 템플릿 (Markdown)
  - 저장: rca-knowledge/docs/analysis/confluence-guides/

- [ ] **P2-3**: Python 스크립트 작성 (선택)
  ```python
  # scripts/github_to_jira.py
  # scripts/markdown_to_confluence.py
  ```

**완료 기준**: 수동 작업 5분 이하로 단축
