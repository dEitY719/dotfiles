...
[파트 독립 영역] (팀 Epic에 직접 연결되지 않는 Story)
  ...
  └─ [AIE 파트] 수명업무 (P-5)       ← 평가 가중치 0% (tracking only)
      └─ (Task 만 동적 생성 — 지시 접수 시점에)
```
...
### Story P-5: 수명업무 (Tracking-only)

**Summary**: `[AIE 파트] 2026 수명업무 - 비계획 지시/긴급 대응 추적`

**팀 Epic 연결**: 직접 연결 없음 — 파트 독립 영역 (tracking only, 평가 가중치 없음)
...
| P-5 | [AIE 파트] 2026 수명업무 - 비계획 지시/긴급 대응 추적 | 0% (tracking) | - | 파트 독립 |
...
총 Story 수: 5 (파트: 평가 4 + tracking 1) + 4 (핵심제품) + 36 (파트원) = **45개**
...
│   └─ [AIE-{letter}] 개인역량강화 (M-4) × 9명
│
└─ [AIE 파트] 수명업무 (P-5)                     ← 신규, tracking only (0%)
    └─ (Task 만 동적 생성 — 지시 접수 시점에)
```
