...
M-1 Summary 는 `[AIE-{letter} O-1 K-?]` 로 되어 있음. `?` 를 담당 핵심제품 번호(1~4)로 치환 필수:
- Agent App Store 담당 → `[AIE-δ O-1 K-1]` 형태
- Alpha Agent 담당 → `[AIE-δ O-1 K-2]` 형태
- Cowork 담당 → `[AIE-δ O-1 K-3]` 형태
- MCP/Skill Hub 담당 → `[AIE-δ O-1 K-4]` 형태
