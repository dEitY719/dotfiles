...
gh:pr, gh:pr-approve, gh:pr-emergency-merge, gh:pr-reply)에 이슈 기반
...
### 영향받지 않는 스킬
`gh:commit`, `gh:pr`, `gh:pr-approve`, `gh:pr-emergency-merge`,
...
3. **Hard stops**:
   ...
   - `reviewDecision != APPROVED` → "not approved yet — `gh:pr-emergency-merge`
     를 쓰세요" 로 안내.
...
### `gh:pr-emergency-merge` 와의 역할 분담
- `gh:pr-merge` — **approved 일 때만** 머지. 요구사항 전부 만족되어야 함.
- `gh:pr-emergency-merge` — approval 우회용 (admin bypass + audit).
