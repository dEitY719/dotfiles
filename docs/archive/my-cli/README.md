> **아카이브 문서 (2026-09-03, #1740 D-2)** — 이 디렉토리는 `docs/feature/my-cli/` 에서 이동한 보관본이다.
> `packages/my-cli` (Ink/React TUI) 는 같은 결정으로 저장소에서 삭제됐다. 사유: `my-help` UX 방향이
> fzf 명령 팔레트로 확정된 이상 Ink TUI 를 되살리는 일은 UX 작업이 아니라 부채 정리이며, 유지 비용만
> 남기 때문이다. 아래 내용은 2026-02 당시 상태 그대로이며 현재 저장소 경로와 일치하지 않는다.
