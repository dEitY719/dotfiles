...
2. **학습·회고가 issue tracker 외부에 격리** — 외부 학습 (YouTube
   /문서) 정리나 PR 회고는 `docs/guide/learnings/` 파일로만 가는데, 파일은
...
| `Lessons` | Open-ended | 외부 학습·재사용 가능한 지식 정리 (YouTube/문서 학습, 패턴, PR 회고) — **Discussion-first** | 변환 없음 — 양 늘면 `docs/guide/learnings/` 로 승격 |
...
재사용 가능한 지식 정리 (외부 학습/패턴/회고) 인가?
  YES -> Discussion: Lessons (Discussion-first, 양 늘면 docs/guide/learnings/ 승격)
...
가치가 충분히 커지면 `docs/guide/learnings/<slug>.md` 로 승격한다. 파일을
먼저 쓰지 않는다.
...
- `docs/guide/learnings/` 와 중복 작성 금지 — 둘 다 만들면 SSOT 가 분산된다.

### 승격 (Discussion -> docs/guide/learnings/)

다음 중 둘 이상이면 `docs/guide/learnings/<slug>.md` 로 추출 후 Discussion
...
- 결정·RCA 처럼 frozen artifact 가 필요한 항목 → `docs/guide/learnings/`
