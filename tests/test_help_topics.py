...
class TestHelpTopicsWithDifferentFormats:
    """Test help topics with different naming formats (dash vs underscore)."""

    @pytest.mark.parametrize("shell", ["bash", "zsh"])
    def test_git_help_with_dash(self, shell_runner, shell):
        """Test git-help (dash format)."""
        result = shell_runner(shell, "git-help")
        assert result.exit_code == 0, f"{shell}: git-help failed"

    @pytest.mark.parametrize("shell", ["bash", "zsh"])
    def test_mytool_help_with_dash(self, shell_runner, shell):
        """Test mytool-help (dash format)."""
    ...
    def test_my_help_invocation_with_subtopic(self, shell_runner, shell):
        """Test my_help() with specific subtopic (e.g., my-help git)."""
        result = shell_runner(shell, "my-help git")
        assert result.exit_code == 0, f"{shell}: my-help git failed"
...
class TestHelpTopicsErrorHandling:
    ...
    def test_my_help_without_args_lists_topics(self, shell_runner, shell):
        """Test my-help without arguments lists available help topics."""
        result = shell_runner(shell, "my-help")
        assert result.exit_code == 0, f"{shell}: my-help with no args failed"
        # Should list multiple help topics
        assert (
            len(result.stdout.split("\n")) > 5
        ), f"{shell}: my-help output seems incomplete"
