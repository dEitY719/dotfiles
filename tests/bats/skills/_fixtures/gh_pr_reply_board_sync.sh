...
#
# Keep this file in sync with SKILL.md Step 6.5 (the block itself lives in
# references/board-sync-in-review.sh.md). If the skill block changes,
# mirror the change here so the bats suite catches drift.
#
# `--repo "$TARGET_REPO"` (#1405): without it the helper resolves the repo
# via `gh repo view`, i.e. whatever `gh repo set-default` picked, not the
# remote Step 1 resolved.
...
gh_pr_reply_board_sync_step65() {
    local PR_NUMBER="$1" PUSHED_FIXES="$2" TARGET_REPO="${3-}"
