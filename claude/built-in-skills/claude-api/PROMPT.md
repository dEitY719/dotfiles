...
(Note: The full skill includes 11 `<doc>` sections with complete code examples for Python Claude API, Agent SDK, batches, files, streaming, tool use, error codes, live sources, models, and tool use concepts. These are language-specific reference docs that get injected based on the detected project language. Due to their length — approximately 2000+ lines total — they are not reproduced in this PROMPT.md. The doc paths are listed below.)

### Included doc paths:
- `python/agent-sdk/README.md` — Agent SDK Quick Start, built-in tools, query(), ClaudeSDKClient, permissions, MCP, hooks, subagents
- `python/agent-sdk/patterns.md` — Custom tools, hooks, subagents, MCP integration, session resumption patterns
- `python/claude-api/README.md` — Client init, basic messages, vision, prompt caching, extended thinking, multi-turn, compaction, error handling
- `python/claude-api/batches.md` — Batch API (100K requests, 50% cost reduction)
- `python/claude-api/files-api.md` — Files API (upload, use in messages, manage)
- `python/claude-api/streaming.md` — Streaming patterns, event types, thinking streams
- `python/claude-api/tool-use.md` — Tool runner, MCP conversion, manual agentic loop, code execution, memory tool, structured outputs
- `shared/error-codes.md` — HTTP error codes reference
- `shared/live-sources.md` — WebFetch URLs for latest docs
- `shared/models.md` — Full model catalog with programmatic discovery
- `shared/tool-use-concepts.md` — Tool definitions, tool choice, server-side tools, structured outputs
