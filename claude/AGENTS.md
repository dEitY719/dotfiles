...
`claude/hooks/post-gh-pr-create.sh` — 위 디스패처가 `gh pr create` 에서 라우팅하는 핸들러. PR 카드를 "In review", `Closes #N` 로 연결된 이슈를 "In progress" 로 project board 동기화한다(#390). #813 의 auto-add race 흡수용 retry-poll 이 인라인이라 PostToolUse 가 median 7.8s / max 48.7s 를 잡아먹었으므로, #1258 부터 **foreground 에는 sync 1회만** 남기고 나머지(board 유무 probe + 2회차 이후 retry-poll + 연결 이슈 동기화)는 detach 된 백그라운드 서브프로세스로 넘긴다 — 자식이 훅의 stdout/stderr 를 잡고 있으면 Claude Code 가 파이프를 계속 기다리므로 `</dev/null >/dev/null 2>&1 & disown` 리다이렉션이 필수다. 테스트는 `POST_GH_PR_CREATE_ASYNC=0` 으로 deferred 구간을 foreground 에 고정해 호출 횟수를 결정론적으로 검증한다. board 유무 probe 가 쓰는 `_gh_project_status_normalize_repo` 호출부는 #724 가드(`command -v` → 경고 + inline split)를 반드시 유지한다 — 훅과 헬퍼는 따로 배포되므로 헬퍼가 그 함수를 아직 모르는 rollout-skew 창에서 가드 없이는 `has_board` 가 조용히 false 가 되어 #813 retry-poll 이 통째로 사라진다(#1414). best-effort, 항상 exit 0.
