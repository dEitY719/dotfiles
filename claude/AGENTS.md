...
`claude/hooks/bash_wait_loop_guard.py` — `PreToolUse:Bash` guard, `settings.json`에 등록됨(#1521). 이 저장소가 `PreToolUse` 이벤트를 배선하는 첫 훅이다. 자작 대기 루프 중 **종료가 구조적으로 불가능한 두 형태만** 좁게 거절한다: (1) `pgrep -f <TOKEN>` 의 리터럴 패턴이 같은 명령 문자열에 다시 등장하는 경우 — `-f` 는 전체 커맨드라인을 매치하므로 자기 자신이 잡혀 `! pgrep` 이 영원히 거짓이 된다(#1521 실측 49분), (2) `tasks/*.output` 을 도는 `until`/`while` + `sleep` 무한 폴링 — 서브에이전트 완료는 하네스가 자동 통지하므로 중복이다. **규칙을 넓히지 말 것**: Monitor 도구 문서가 `until grep -q "Ready in" dev.log; do sleep 0.5; done` 형태를 권장하므로 일반 폴링 루프는 반드시 통과해야 하고, `tests/integration/test_bash_wait_loop_guard.py` 가 그 명령을 ALLOWED 로 고정한다. 거절 사유는 문자클래스 회피(`[a]7ab…`)를 권한다 — Bash 도구가 명령을 `zsh -c … eval '<command>'` 래퍼로 감싸 자기매칭이 2건(안쪽 셸 + 바깥 래퍼) 잡히므로 `| grep -vx "$$"` 만으로는 부족하다(실측). 출력 스키마는 Stop 훅들의 레거시 `{"decision":"block"}` 이 아니라 현행 `hookSpecificOutput.permissionDecision:"deny"` + exit 0 다. 탈출구 `BASH_WAIT_LOOP_GUARD_BYPASS=1`, 트레이스 `BASH_WAIT_LOOP_GUARD_TRACE=1`. 모든 예외에 fail-open, 항상 exit 0.
