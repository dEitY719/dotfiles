<!-- .company/steering/permissions.md 파일 내용 -->
...
```yaml
auto_execute:
  - bugfix                # 즉시 대응 가능한 버그 수정
  - test                  # 테스트 추가·수정
  - refactor              # 리팩토링
  - docs                  # 문서 업데이트
  - dependency_patch      # 패치 버전 업데이트

ceo_approval:
  - new_feature           # 신기능은 CEO 방향성 확인 필요
  - architecture_change   # 아키텍처 변경은 영향 범위가 큼
  - deploy_production     # 프로덕션 배포는 항상 CEO 확인
  - major_update          # 메이저 버전 업데이트
...
  production: draft # 프로덕션은 항상 CEO 승인
```
...
  client_communication: draft      # 클라이언트 연락은 CEO 확인
  marketing_email: draft # 마케팅 이메일도 CEO 확인
...
1. 자동 실행 태스크가 3회 연속 실패한 경우 → CEO 알림
2. 예산의 80%에 도달한 경우 → CEO 알림
3. 보안 인시던트 의심 시 → 즉시 CEO 알림
4. 사용자로부터 클레임 접수 → 즉시 CEO 알림
