...
matter, so `/gh-commit 123`, `/gh-commit upstream`, `/gh-commit 123 upstream`
and `/gh-commit upstream 123` all resolve correctly. The remote defaults to
...
- `/gh-commit 123` — same, but force `Closes #123` in the footer
- `/gh-commit upstream` — same as bare, but the ai-metrics comment and board
  sync target `upstream`'s repo/host instead of `origin`'s
- `/gh-commit 123 upstream` — force `Closes #123` and target `upstream`
...
- **Good**: `/gh-commit 42` — same, but forces `Closes #42` regardless of chat.
- **Good**: `/gh-commit 42 upstream` — same, but the metrics comment and board
  sync go to `upstream`'s repo (this is what `gh:issue-flow <N> upstream`
  passes down).
