# gh:commit — Positional Args and GitHub Target (#1403, #1405)
...
`/gh:commit [issue-number] [remote]` — a positional made only of digits is the
issue number, any other positional is the remote name. `/gh:commit 123`,
`/gh:commit upstream`, `/gh:commit 123 upstream` all work; bare `/gh:commit` is
...
calls `gh` on its own — and it is what makes `/gh:commit <N> upstream` sync
