...
After the skill is finalized (user is satisfied, packaging done or skipped), run `/skill-check` on the new SKILL.md. If any check returns FAIL or WARN, immediately run `/skill-refactor` to bring it under 100 lines with proper Progressive Disclosure structure. Report the before/after line counts to the user.
...
- Run `/skill-check` → if FAIL/WARN, run `/skill-refactor` (Progressive Disclosure quality gate).
