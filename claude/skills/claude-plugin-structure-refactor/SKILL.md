...
description: >-
  ...
  structure", "/claude-plugin:structure-refactor". Edits —
  `claude-plugin:structure-check` only audits.
compatibility:
...
`claude-plugin:structure-check` (audits what this fixes) · `claude-plugin:rename-repo` (renames the repo to the team convention) · `claude-plugin:create` (builds a new repo from scratch).
