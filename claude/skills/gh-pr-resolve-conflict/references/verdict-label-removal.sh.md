...
## `context-mode` 인자를 넘기지 않는 이유 (#1704)

#1704 는 공유 헬퍼에 아홉 번째 `[context-mode]` 인자를 추가해, `changed` 판정에
한해 context 를 뺀 `git diff -U0` 비교로 한 번 더 물어보는 완화 경로를 만들었다.
**이 스킬은 그 인자를 일부러 넘기지 않는다** — 기본값 `strict` 그대로, context
포함 비교만 쓴다. 위 두 스니펫이 여덟 번째 인자에서 끝나는 것은 누락이 아니라
설계다.

이유: 이 스킬의 리베이스는 기계적이라고 보장할 수 없다. 충돌 없이 끝나는
경우도 있지만(그래서 #1700 이 헬퍼를 공유하게 했다), **사람이 충돌 hunk 를 손으로
편집한 뒤** 완료된 경우도 똑같이 있다. 그 경우 patch-id 일치는 — context 를 빼든
말든 — PR 자신의 diff 가 안 바뀌었다는 것만 증명할 뿐, 사람이 내린 수동 해소가
새 base 위에서 안전하다는 것까지 증명하지 못하고, 모델은 그걸 보증할 근거가
없다. 최소한 엄격 비교는 "주변이 움직였다"는 신호라도 남긴다.
`gh:pr-resolve-outdated` 는 구조적으로 이 경우가 없다 — `git rebase` 가 멈추는
순간 이 스킬로 넘기므로 그쪽 리베이스는 100% 기계적이다.
