...
description: >-
  ...
  diverged from base — not just HEAD. Use for /gh:pr, /gh-pr, "PR 생성", "풀리퀘
...
Issue precedence, same as `gh:commit`: (1) explicit `/gh:pr <N>`, (2) recent
...
`references/pr-body-template.md`. After it (all-missing no-op included), emit `printf '[step:gh-pr/labels] OK\n'`.
...
defense-in-depth `Board sync:` row, #747, included) and the closing `[step:gh-pr/report] OK` marker. No extra summary — the user opens the URL.
...
`gh:commit` makes the commits this PR bundles · `gh:pr-review` / `gh:pr-approve` review it · `gh:pr-merge` merges it.
