...
- `/devx-pr-verify-live` — 현재 커밋이 속한 PR 을 자동 역추적해 검증
- `/devx-pr-verify-live 2483` — PR #2483 을 검증하고 발견을 대상 레포 이슈로 등록
- `/devx-pr-verify-live 2483 --url http://127.0.0.1:5173/` — 발견 단계를 건너뛴다
- `/devx-pr-verify-live 2483 --matrix full` — 축을 줄이지 않고 전체 곱을 훑는다
...
- **Good**: `/devx-pr-verify-live 2483` — 머지 직후, 서버는 이미 떠 있는 상태.
- **Good**: `/devx-pr-verify-live 2483 --api-url http://127.0.0.1:8000` — 같은 호스트에
