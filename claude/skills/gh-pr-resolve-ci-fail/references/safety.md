...
`review-passed` is a claim about **one head commit**, and the push above just
landed a new one — so the verdict is now false. Drop it **immediately after
this push succeeds** — not deferred to Step 7 — through the shared
`_gh_pr_drop_label` helper (the single REST-DELETE implementation every
head-advancing skill routes through, never a hand-inlined REST DELETE; #1563,
and #326 Bug B for the add side). The helper percent-encodes the label, pins
`GH_HOST`, and absorbs "label was never there" as success after verifying the
PR's real label list (#1583) — no pre-check needed.
...
**Why here and not in Step 7 (#1711 codex review, BLOCKER)**: Step 6 is an
*optional* `--wait` poll that can run for minutes with the fix's head
unchanged. If the drop waited until Step 7, a re-review completing during
that window could grant a **freshly valid** `review-passed` for this exact
head, and the deferred drop would then delete a verdict that was correct at
the moment it ran. Dropping right here — before Step 6 ever starts polling —
removes the window entirely: nothing can grant a verdict for this head before
this line executes, because this line runs the instant the head that
invalidates the old verdict exists.
...
changes the CI-fix success/failure or the final report. This block sits
behind the same gate as the push above: push failed → this block does not run
at all, and the label stays valid because head never moved.
...
The `review-passed` drop is **not** here — it runs right after Step 5's push,
before Step 6's optional `--wait`. See "`review-passed` drop" under Step 5
push above, and the #1711 rationale for why it moved.
