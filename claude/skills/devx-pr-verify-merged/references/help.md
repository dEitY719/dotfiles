...
exit 2), 아니면 remote 다. 그래서 `/devx-pr-verify-merged upstream` 처럼 **PR 은 자동 감지하고
...
검증이 필요하면 `devx:pr-verify-live` 를 쓴다.

## Usage

- `/devx-pr-verify-merged` — 현재 커밋이 속한 머지된 PR 을 자동 역추적해 검증
- `/devx-pr-verify-merged 16` — PR #16 을 신선한 클론에서 검증하고 발견을 대상 레포 이슈로 등록
- `/devx-pr-verify-merged 16 --matrix full` — 유도된 환경 축을 전부 실행
- `/devx-pr-verify-merged 16 --env path,eol` — PATH 정규화와 줄 끝 축만 실행
- `/devx-pr-verify-merged 16 --clone-dir ~/tmp/verify-16` — 클론 위치 고정(정리는 사용자 몫)
- `/devx-pr-verify-merged 16 --no-diff-check` — 차등 검증을 끄고 클론 재현성만 본다
- `/devx-pr-verify-merged 16 --dry-run` — 이슈 본문만 출력하고 등록하지 않는다
- `/devx-pr-verify-merged 16 --no-issue --no-comment` — 로컬 리포트만
- `/devx-pr-verify-merged upstream` — PR 은 자동 감지, remote 만 `upstream` 으로
- `/devx-pr-verify-merged -h` / `--help` / `help` — 이 도움말
...
- **Good**: `/devx-pr-verify-merged 16` — 머지 직후, shell/CLI/라이브러리 레포에서.
- **Good**: `/devx-pr-verify-merged 16 --matrix full` — 줄 끝·로케일이 걸린 PR 이라 축을 다 돈다.
- **Bad**: 작업 worktree 에서 테스트를 직접 돌려 "검증했다" 라고 하는 것 — 이 스킬이 막으려는
  실패 그 자체다(untracked 픽스처가 있으면 클론에서는 그 케이스가 아예 실행되지 않는다).
- **Bad**: `/devx-pr-verify-merged 16 --env=` — exit 2. 명시한 override 를 조용히 무시하지 않는다.
- **Bad**: 미머지 PR 에 이 스킬을 부르는 것 — 정지한다. `devx:pr-verify-live` 를 쓴다.

## 관련 스킬

| 스킬 | 관계 |
|---|---|
| `devx:pr-verify-live` | **sister skill**. 같은 머지 후 슬롯, 다른 증명 대상(서빙 체크아웃 동일성). 공통 계약은 `../devx-pr-verify-live/references/verify-contract.md` 한 파일 |
| `gh:issue-create` | 발견 1건마다 호출. 본문 골격 · 라벨 · ai-metrics 의 SSOT |
| `gh:issue-implement` | 테스트 러너 탐지 사다리의 출처. 여기서 그대로 재사용한다 |
| `devx:pr-review-all` | 머지 **전** 정적 리뷰 게이트. 이쪽은 머지 **후** 재현성 검증 |
