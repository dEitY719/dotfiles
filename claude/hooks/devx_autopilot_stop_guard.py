...
# match — which would hand the #1275 over-count straight back for this one
...
def _count_fresh_user_prompts(messages: list[dict[str, Any]], start: int) -> int:
    ...
    A `role=user` transcript entry counts only when ALL of these hold:
      ...
      - the remainder starts no line with `_SKILL_EXPANSION_RE` (an invoked
        sub-skill's body injected as a user message — Stage-B expands
        several) nor `_HARNESS_INJECTION_RE` (Stop-hook feedback,
        background-task notifications) — both are machinery generated by the
        flow itself. Line-anchored, not substring (#1281): a human quoting
        one of those strings mid-sentence is still a fresh prompt.
