...
def _count_fresh_user_prompts(messages: list[dict[str, Any]], start: int) -> int:
    ...
    A `role=user` transcript entry counts only when ALL of these hold:
      ...
      - the remainder contains none of `_SKILL_EXPANSION_MARKERS` (an
        invoked skill's body injected as a user message) nor any of
        `_HARNESS_INJECTION_MARKERS` (Stop-hook feedback, background-task
        notifications) — both are machinery generated by the flow itself.
